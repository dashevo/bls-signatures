...
PrivateKey PrivateKey::FromSeed(const uint8_t* seed, size_t seedLen) {
    ...
    const uint8_t L = 48;  // `ceil((3 * ceil(log2(r))) / 16)`, where `r` is the order of the BLS 12-381 curve
    ...
    HKDF256::ExtractExpand(okmHkdf, L, ikmHkdf, seedLen + 1, saltHkdf, 20, keyInfoHkdf, infoLen + 2);
